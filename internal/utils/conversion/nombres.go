...
var NombresEnLettres = map[string]int{
	...
	"quarante-huit": 48, "quarante-neuf": 49, "cinquante": 50, "cinquante-et-un": 51,
	...
	"soixante-huit": 68, "soixante-neuf": 69, "soixante-dix": 70, "soixante-onze": 71,
	"soixante-douze": 72, "soixante-treize": 73, "soixante-quatorze": 74, "soixante-quinze": 75,
	"soixante-seize": 76, "soixante-dix-sept": 77, "soixante-dix-huit": 78, "soixante-dix-neuf": 79,
	"quatre-vingts": 80, "quatre-vingt-un": 81, "quatre-vingt-deux": 82, "quatre-vingt-trois": 83,
	"quatre-vingt-quatre": 84, "quatre-vingt-cinq": 85, "quatre-vingt-six": 86, "quatre-vingt-sept": 87,
	"quatre-vingt-huit": 88, "quatre-vingt-neuf": 89, "quatre-vingt-dix": 90, "quatre-vingt-onze": 91,
	"quatre-vingt-douze": 92, "quatre-vingt-treize": 93, "quatre-vingt-quatorze": 94, "quatre-vingt-quinze": 95,
	"quatre-vingt-seize": 96, "quatre-vingt-dix-sept": 97, "quatre-vingt-dix-huit": 98, "quatre-vingt-dix-neuf": 99,
	"cent": 100,
}
...
func LettreVersEntier(mot string) (int, bool) {
	...
	// Tentative directe dans la map
	if v, ok := NombresEnLettres[mot]; ok {
		return v, true
	}

	// Tentative en tant que chiffre ASCII
	var n int
	for _, c := range mot {
		if c < '0' || c > '9' {
			return 0, false
...
func ChiffreVersLettre(nom string) string {
	inverse := make(map[int]string, len(NombresEnLettres))
	for lettre, chiffre := range NombresEnLettres {
		if existing, ok := inverse[chiffre]; !ok || len(lettre) < len(existing) {
			inverse[chiffre] = lettre
		}
	}

	// Remplacer chaque token numérique trouvé dans le nom
